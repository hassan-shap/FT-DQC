...
loss_prob_list = [0.95] # np.linspace(0.1,0.8,5)
...
for loss_prob in loss_prob_list:
    ...
    for i_d, d in enumerate(dist_list):
        ...
        dy = d
        repetitions = d
        code_details_2d = surface_code_plaquette(dx, dy)
        code_details_3d = surface_code_3d_plaquette(dx, dy,repetitions)
        ...
        def runner(i_rep):
            tic = time.time()
            logical_err_rate = joint_meas_2d_phase_diagram(code_details, params)
            out_dir = "results/joint_meas_pd_plaq/"
